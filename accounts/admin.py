...
class CustomUserAdmin(UserAdmin):
    ...
    model = User
    list_display = ['username', 'email', 'first_name', 'last_name', 'role', 'designation', 'is_active', 'date_created']
    list_filter = ['role', 'is_active', 'is_staff', 'date_created']
    search_fields = ['username', 'email', 'first_name', 'last_name', 'phone', 'designation']
    
    fieldsets = (
        (None, {'fields': ('username', 'password')}),
        ('Personal Info', {'fields': ('first_name', 'last_name', 'email', 'phone', 'designation', 'profile_picture')}),
